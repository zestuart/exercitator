...
- **Containers**: `exercitator-mcp` (MCP server + HTTP API on 8643) + `tailscale-exercitator` (funnel sidecar) + `praescriptor-web` (web UI) + `tailscale-praescriptor` (serve sidecar) + `exercitator-api-ts` (serve sidecar for HTTP API) + `garmin-bridge` (Python/garth Garmin Connect recovery + FIT source on 8655, read internally over the compose network) + `garmin-bridge-ts` (serve sidecar for out-of-band bridge access at `garmin.tail7ab379.ts.net`).
- **Volumes**: `exercitator-data` (SQLite), `exercitator-tailscale-state` (external), `praescriptor-tailscale-state` (external), `exercitator-api-tailscale-state` (external), `garmin-token-state` (external — the garth OAuth token dir mounted into the bridge at `/tokens`), `garmin-tailscale-state` (external). Do not delete external volumes — Tailscale node identity + the Garmin token live in them. Pre-create the newer external volumes on Cogitator once: `ssh dominus@cogitator.tail7ab379.ts.net 'zsh -ic "docker volume create exercitator-api-tailscale-state garmin-token-state garmin-tailscale-state"'`.
- **Garmin token bootstrap** (one-time, before the Garmin source works): Garmin login needs email + password + MFA, so it cannot run headless. Mint the garth token locally and copy it into `garmin-token-state` — full runbook in `garmin-bridge/README.md`. Until then the bridge returns `garmin_reauth_required` and ze's `auto` health source falls through to the WHOOP hard-fail (no regression).
- **Tailscale auth**: all four sidecars (`tailscale-exercitator`, `tailscale-praescriptor`, `exercitator-api-ts`, `garmin-bridge-ts`) authenticate with the shared **non-expiring Tailscale OAuth client** secret (`tskey-client-…?ephemeral=false&preauthorized=true`, scoped `tag:mcp`) — the same client the internuntius core-seven use. Converted from an expiring `tskey-auth-…` key 2026-07-13 (see `lessons.md`). `TS_EXTRA_ARGS=--advertise-tags=tag:mcp --accept-dns=false` is **mandatory**: an OAuth key requires an advertised tag, and `--accept-dns=false` keeps each sidecar resolving its Docker-network upstream (`exercitator-mcp:8642`) rather than tailnet DNS. Every sidecar's compose `environment:` must forward `- TS_EXTRA_ARGS=${TS_EXTRA_ARGS:-}` or `tailscale up` aborts. The secret lives only in `.env` and `praefectura/docs/tailscale.md` — never echo it (or any prefix of it) into source-controlled docs.
...
| `TAILSCALE_AUTH_KEY` | yes | tailscale-* sidecars | non-expiring Tailscale **OAuth client** secret, scoped `tag:mcp` (value in praefectura/docs/tailscale.md) |
| `TS_EXTRA_ARGS` | yes | tailscale-* sidecars | `--advertise-tags=tag:mcp --accept-dns=false` — mandatory with the OAuth key; compose must forward `- TS_EXTRA_ARGS=${TS_EXTRA_ARGS:-}` per sidecar |
...
| `GARMIN_BRIDGE_API_KEY` | optional | garmin-bridge, exercitator, praescriptor | shared bearer between the Garmin bridge and its consumers. ze's `healthSource: "auto"` uses Garmin (Body Battery / HRV / sleep) as the fallback when WHOOP is absent. Unset → Garmin source unavailable (auto degrades to a WHOOP-only hard-fail). The garth OAuth token is a mounted volume, not an env var — see `garmin-bridge/README.md` |
| `GARMIN_URL` | optional | exercitator, praescriptor | Garmin bridge base URL; defaults to the compose-internal service `http://garmin-bridge:8655` |
