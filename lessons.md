...
**Prevention**:
- Any function that calls `localDateStr` / `fetchTrainingData` / `generatePrescriptions` must thread the athlete `tz` through — a missing `tz` silently means container-UTC, which diverges from every tz-aware surface after local-evening (west of UTC) or before local-morning (east). This is the **third** UTC-vs-athlete-tz defect (cf. the 2026-06-03 `/status` window and the WHOOP 18-minute-night entries). When a function recomputes something a tz-aware caller already produced, it must take the same `tz`.
...
- Before validating field extraction against a third-party API of unknown shape, make one live call through the *writer's* environment and inspect the actual JSON (Body Battery entry was `[epoch_ms, level]` 2-element, not the assumed 3-element `[ts, status, level]`). Defensive extraction that scans for "the 0–100 int anywhere in the row" survived the wrong assumption; hard-coded indices would not have.
- Any `docker run -v <host-path>:<ctr>` against Cogitator/Colima must stage `<host-path>` under `$HOME`, never `/tmp` — same rule as build contexts.
- Verify a token-clone deploy end-to-end by *forcing* the new source (the WHOOP→Garmin fallback won't exercise Garmin if WHOOP happens to be healthy that day, as it was here after the strap resumed).

## 2026-07-12 — Garmin native FITs use standard fields (not CIQ dev fields); never commit a raw device FIT to a public repo

**What happened**: Building Vigil support for Garmin-recorded runs (Phase 2), two things had to be right before writing the extractor. (1) Garmin's original FIT exposes running dynamics as *standard* snake_case fields (`stance_time`, `stance_time_balance`, `vertical_oscillation`, `power`, `cadence`+`fractional_cadence`), whereas the existing Stryd extractor gates on CIQ *developer* fields ("Leg Spring Stiffness", "Form Power", "Impact"). A Garmin FIT has none of those, so the Stryd extractor correctly no-ops on it — but a naive reuse would have extracted nothing. (2) A real ze Garmin FIT embeds home GPS (`position_lat`/`position_long` ≈ 51.457, -0.120) on every record, and the exercitator repo is **public**.

**Root cause**: (1) Garmin writes running dynamics to the standard FIT profile; Stryd writes proprietary metrics (LSS, Form Power, ILR) as CIQ developer fields with human-readable names. They are different namespaces, and the Garmin metric set is a strict *subset* (no LSS/Form Power/ILR). (2) Committing a device FIT verbatim would leak home coordinates into public git history permanently — the same class of harm as committing a secret.

**Fix**: Pulled a real ze run FIT via the blueToothDisco garth venv, parsed it with the shipped `parseFitBuffer`, and inspected the actual field names *before* writing `garmin-fit.ts` (per the RESUME's "don't guess field names"). Built a Garmin extractor for the available subset — GCT asymmetry comes from Garmin's native `stance_time_balance`, matching the Stryd Duo's GCT balance (ze runs a Duo, not a single pod; the Duo also gives LSS/VO/ILR balance, which Garmin does not — so Garmin is a subset on asymmetry too). Committed the fixture as **GPS-stripped, gzipped parsed records** (`tests/fixtures/garmin/garmin-run-records.json.gz`, 43 KB) — no coordinates, and tests run the extractor on the records while mocking the bridge client. Also chose **per-source baselines** (`source` in `vigil_metrics` + `vigil_baselines` PK) so a wrist-watch GCT offset can't contaminate the foot-pod baseline.

**Prevention**:
- Before extending a vendor-specific parser to a new vendor, obtain one real artefact and inspect the actual field names — vendors that both "record runs" can use entirely different FIT namespaces (standard vs developer fields). Verified by the `garmin-fit` test asserting real values (GCT 287.6 ms, asymmetry 1.87%) against a genuine FIT.
- Never commit a raw device capture (FIT/GPX/TCX) to a public repo — strip GPS (and any PII) and prefer a parsed-records fixture over the binary. Treat home coordinates like a secret.
- When merging biomechanics from two recording devices for the same athlete, keep baselines per-source unless the devices are proven to agree — a systematic offset inflates stddev and corrupts z-scores. `runVigilPipeline` returns the worst active source (injury-conservative).

## 2026-07-13 — Re-tagging an already-tagged Tailscale node mints a new identity (`-1` collision); manual `tailscale set` crash-loops the sidecar

**What happened**: Converting exercitator's four Tailscale sidecars from the expiring `tskey-auth-…` key to the shared non-expiring OAuth client (`tag:mcp`, matching the internuntius core-seven) took down all four Funnel ingresses. The MCP endpoint (`exercitator.tail7ab379.ts.net`) went from a healthy 401 to a connection **timeout** — its clean Funnel DNS record vanished entirely. Compounding it, a manual `tailscale set --hostname` run mid-recovery then crash-looped all four sidecars.

**Root cause**: (1) The existing nodes were already *tagged* (under a different owner). Re-registering them with `--advertise-tags=tag:mcp` via the OAuth client minted **new node identities** rather than re-authing in place — unlike a user-owned→tagged conversion (the core-seven case, which kept identity because the machine key persisted in the state volume). The old identities became offline orphans still holding the clean hostnames, so the live nodes registered as `exercitator-1`/`praescriptor-1`/etc.; Funnel had only ever published DNS for the clean name (owned by the now-offline orphan) → public timeout, **not** a 502. (2) `tailscale set --hostname` on a containerboot-managed node persists a pref that containerboot's next `tailscale up` doesn't mention, tripping the "must mention all non-default flags" guard → `up` exits 1 → `restart: unless-stopped` loops. `tailscale set --hostname` also does **not** drop an already-assigned `-1` MagicDNS suffix.

**Fix**: (1) Deleted the four offline orphan nodes in the Tailscale admin console — this needs the console or a devices-scoped API token; the OAuth client's `auth_keys` scope cannot delete devices — which freed the clean names. (2) Cleared the polluted prefs with a one-time `TS_EXTRA_ARGS=--reset --advertise-tags=tag:mcp --accept-dns=false` boot (`docker compose up -d --force-recreate` using **service** names `tailscale-exercitator-api`/`tailscale-garmin`, not container_names), which also reclaimed the now-free clean hostnames, then reverted `TS_EXTRA_ARGS` to the stable `--advertise-tags=tag:mcp --accept-dns=false`. Verified: all four on clean hostnames, `tag:mcp`, `/mcp`→401, `restarts=0`.

**Prevention**:
- Converting an **already-tagged** node to a different tag owner is not an in-place re-auth — it mints a new identity and will `-1`-collide. Expect to delete the old node in the admin console as part of the change; the core-seven runbook's "keeps hostnames, no `-1`" only holds for user-owned→tagged. Keep a devices-scoped Tailscale API token to hand so orphan cleanup needs no human console step.
- Never run `tailscale set` / `tailscale up` by hand inside a containerboot-managed sidecar — it drifts persisted prefs out of sync with the env-derived `up` command and crash-loops the next boot. Fix drift with a one-shot `--reset` boot via `TS_EXTRA_ARGS`, then revert.
- `--accept-dns=false` is load-bearing for these sidecars: they must resolve the Docker-network upstream (`exercitator-mcp:8642`) via Docker DNS, not tailnet DNS.
- `docker compose up -d <name>` takes **service** names, not `container_name`s — passing a container_name aborts the whole command ("no such service") and recreates nothing.
- A vanished Funnel is a **missing public DNS record** (connection timeout / `000` on curl), distinct from a 502 (ingress up, upstream down). `dig +short @1.1.1.1 <host>.tail7ab379.ts.net` returning empty is the tell.

## 2026-07-14 — Distance-based Stryd workout rendered "0-minute" intervals

**What happened**: The Praescriptor dashboard showed ze's 2026-07-14 run with four **0-minute interval rows**. The Stryd `long` bucket had served "The Tom Workout (Distance)" — 1-mile work/rest reps prescribed by **distance**, not time.

**Root cause**: `strydWorkoutToSegments` (`src/engine/stryd-mapper.ts`) mapped any segment with `duration_type === "distance"` to `duration_secs = 0` — a deliberate "not observed on our account, deferred until seen" stub whose comment asserted Stryd shipped only time-based segments. That assumption was false: a Stryd **library template** can prescribe distance reps, and the unit (`distance_unit_selected`, `"mile"` here) is a **per-segment property of the authored template**, not an account/locale setting. The bug had depth: `total_duration_secs` is recomputed as the segment-seconds sum (`stryd-swap.ts`), so it collapsed to WU+CD only (1320 s); the write-once persist (`persist.ts` — `if (existing) return`) froze the broken 07:25 render as the day's record even though a later render that day chose a valid time-based workout; and send-to-intervals would have emitted "0min" steps (the 422 guard only checks `status`, not zero-duration). Send-to-Stryd was insulated (round-trips the raw body).

**Fix**: Added `distanceToMetres(value, unit)` (unit-aware; unknown → metres + warn) and populated `distance_m` + `duration_type: "distance"` on distance segments (`duration_secs` stays 0). Render shows metric km (never "0min") plus a distance meta-pill; intervals.icu emits `mtr` (not `m` = minutes); a defensive distance branch was added to the flattened `stryd-format` fallback. Metric-only per ze's directive — the raw miles never surface (Stryd keeps them via the raw-body round-trip). Committed the real workout body as a public-repo-safe fixture (template body — no GPS).

**Prevention**:
- New tests: `distanceToMetres` unit conversions + unknown-unit fallback; `strydWorkoutToSegments` distance mapping; stryd-swap e2e on the distance fixture; dashboard render asserts "1.61 km" and **no "0min"** in the run card; intervals-format asserts `1609mtr`. Regression is now caught by the committed distance fixture.
- **A "deferred until observed" stub for external-vendor data is a latent bug, not a safe placeholder.** When a field is plumbed-but-unhandled, the guard should degrade *visibly and correctly* (here: surface the distance we already have), not emit a value (0) that other layers silently propagate into totals, load, persistence, and send paths.
- **Known limitation left open**: `total_duration_secs`/`estimated_load` still understate a distance workout (distance reps carry no seconds). The durable fix is to thread Stryd's per-segment `estimated_workout.estimates[].segment_estimates[].duration` (available at swap time via `pickStrydWorkout`) — deferred by decision; if a distance workout is ever *executed*, compliance grading of the 0-second prescribed segments (`assess.ts`) also needs distance-vs-actual matching.
- **Second-order, noted not fixed**: write-once persist vs last-write DSW emit produced two different records for 2026-07-14 (persisted "The Tom Workout (Distance)"/long at 07:25 ≠ DSW "Power Progression Ladder"/threshold at 15:32). The day's "record of what was prescribed" is inconsistent across surfaces when the category drifts intraday.
