...
- **Power-source override endpoint**: `POST /:userId/api/power-source` (Praescriptor, tailnet-only) allowlists `source` to `{auto, stryd, garmin}` (400 otherwise) before it reaches SQLite `user_preferences`; no path interpolation, no user-controlled key/value. Classified as a "write" so the 10/min per-user bucket applies. Persisted value is re-validated on read (`getPowerSourceOverride` returns `null` for any non-`{stryd,garmin}` value), so a corrupt row degrades to auto, never to an injected source.
- **Health-source override endpoint**: `POST /:userId/api/health-source` (Praescriptor, tailnet-only) allowlists `source` to `{auto, promus-whoop, garmin}` (400 otherwise) → `setHealthSourceOverride` → `user_preferences`. Same posture as the power-source endpoint: write-rate-limited, no path interpolation, re-validated on read (a corrupt row → profile default, never an injected source).
...
- **Garmin bridge client + service** (`src/garmin/client.ts` + `garmin-bridge/`): GET-only bearer client reading Garmin Connect recovery telemetry + activity FITs from the in-house `garmin-bridge` sidecar (`GARMIN_BRIDGE_API_KEY`; base URL `GARMIN_URL`, default the compose-internal `http://garmin-bridge:8655`). Same client posture as Promus: 512 KiB JSON cap (16 MiB FIT cap), 20 s timeout, `encodeURIComponent`-escaped params, no secret logged. The bridge itself is bearer-gated (constant-time compare, fail-closed when the key is unset), tailnet-only (no funnel), and holds a **garth OAuth token in a mounted volume** (`garmin-token-state`) — minted out-of-band (Garmin MFA can't run headless), never committed, never an env var. Garmin's unstable third-party API is deliberately isolated in this one service; a token expiry/revocation surfaces as `garmin_reauth_required` (503), not a crash. Garmin data is treated as untrusted-shape input (defensive field extraction).
...
**2026-07-12 extension** (Garmin recovery Phase 2 SAST diff): the same buffer-before-cap pattern applies to the **binary FIT downloads** — `StrydClient.downloadFit` (`arrayBuffer()` then 10 MB check) and `GarminClient.getActivityFit` (`arrayBuffer()` then 16 MB check), reached by the new Vigil backfill. Accepted as part of #37, unchanged rationale (trusted authenticated single-user upstreams). Extra mitigations specific to the FIT path: the Garmin FIT is **additionally capped bridge-side before transmission** (`garmin-bridge/garmin_client.py` rejects on the zip's declared uncompressed `file_size` > 16 MB before `zf.read`), and **FIT is an uncompressed record stream** — no decompression-bomb amplification. The proper fix is the same streaming bounded read; the binary variant should be folded into the #37 work. Two *new* Phase 2 DoS guards (Vigil-query 800-day range cap in `db.ts`; `MAX_RECORDS` 200k cap in both FIT extractors) were fixed, not accepted — re-scan surfaced only this #37 residual.
