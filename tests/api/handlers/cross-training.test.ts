...
describe("handleCrossTrainingRpe", () => {
	it("writes RPE to intervals.icu and returns strain tier", async () => {
		...
		// (Foster's RPE × duration_in_MINUTES, what the strain cascade reads).
		// moving_time is in seconds, so 6 × 2400 / 60 = 240.
		...
		// session_rpe = 240 → > 200 → moderate tier
		expect(body.strain_tier).toBe("moderate");
	});
