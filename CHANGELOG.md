...
- HTTP API `critical_power.watts` and `critical_power.source` can no longer disagree on which engine produced the headline number (closes #31). Previously the staleness override path could set `watts` from intervals.icu rolling FTP while `source` still reported `stryd_direct`. Removing the override (see Changed) makes `watts === Math.round(strydCp.cp)` whenever Stryd creds are configured, by construction. The earlier 2026-05-08 patch that flipped `criticalPowerFromContext` precedence to prefer `powerContext.ftp` over `strydCp` is now defensive-only — both arguments are equal on the Stryd path.
