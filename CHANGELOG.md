...
### Added
- **Stryd round-trip push** (`src/web/stryd-format.ts`, `src/stryd/client.ts`). The Send-to-Stryd button on Stryd-sourced run cards now creates a faithful copy of the served workout on the athlete's Stryd calendar by round-tripping the original Stryd workout payload verbatim (block structure, repeat counts, exact `intensity_percent` bands). `WorkoutSuggestion.strydOriginalWorkout` carries the original payload through the swap layer; `toStrydWorkout` emits it directly when present, falling back to the engine-build converter otherwise. The earlier "Send to Stryd suppressed on Stryd-sourced cards" gate is removed — push is verified byte-faithful. Verified live 2026-05-25: pushed Dash & Dine landed with `stress=25.24, duration=1560s, intensity_zones=[930,0,480,150,0]` matching the original recommendation exactly.
- **`scheduleWorkout` TZ fix** (`src/stryd/client.ts`). Removed the `setHours(0,0,0,0)` floor that set midnight in the JS runtime's local TZ. In a UTC production container that turned a 14:00 PDT push into "yesterday" in user TZ. Stryd renders the timestamp in user TZ; "now" always renders as today during the user's daytime. Long-standing latent bug affecting any user west of UTC for any push after their local 17:00.
- **PATCH `selected_id` preference signal on every send** (`src/stryd/client.ts:markRecommendationSelected`, `src/web/mark-stryd-selected.ts`). Whenever ze taps Send-to-Stryd OR Send-to-intervals.icu on a Stryd-sourced run, a fire-and-forget PATCH fires back to Stryd telling them which workout was picked. Stryd's recommendation engine learns the user's choice regardless of execution channel. Verified empirically that the PATCH is state-only — Stryd flips `selected_id` in the recommendation set but does NOT auto-schedule, so it complements (rather than replaces) the round-trip push. `recommendationSetId` validated via `^\d+$` before URL interpolation to defend against path injection from a compromised upstream. 6 new vitest cases.
- **Stryd swap applied on every Run-producing HTTP API surface** (`src/api/handlers/workouts.ts`, `src/api/handlers/dashboard.ts`). Praescriptor's `generatePrescriptions` already applied the swap; the HTTP API endpoints `GET /workouts/suggested` and `GET /dashboard` were calling `suggestWorkoutFromData` directly and returning engine output. Wiring extracted into `applyStrydSwapIfEnabled` in `src/web/stryd-swap.ts` (same gate: sport=Run, status!=awaiting_input, profile flag, client present, FTP>0). All consumers now agree. DSW emission fires from each surface; Promus dedups via `(user_id, date, sport, source)` natural-key upsert.
- **Engine narrative stripped on Stryd-sourced suggestions** (`src/web/stryd-swap.ts`). The swap replaces `suggestion.rationale` with the Stryd workout's own `desc` (e.g. "This is a mixed aerobic and speed development workout…"), neutralises `terrain` to `"any"` + `terrain_rationale: ""`, and filters warnings to drop staleness/buffer narrative (the "Adding 10s/X buffer" + "easing back in" + "thresholds may have regressed" patterns from `src/engine/staleness.ts:88,100`). Health-related warnings (sleep, HRV, TSB, Vigil, power-source) survive — they describe athlete state, not engine output. 2 new vitest cases.
- **Pair-collapse mirrored on the API surface** (`src/api/payload.ts`, `src/api/types.ts`). API consumers (Excubitor, Nunc, future clients) now see one collapsed "5x Work set" entry instead of 10 flat (work, rest) segments for fartlek-style blocks. `ApiSegment` gains optional `rest_target` + `rest_target_description` so the rest portion's distinct power band (60–70 % CP) is on the wire. Engine-built intervals leave the new fields unset; existing consumers ignore them. `groupPairSegments` + `SegmentGroup` lifted from `src/web/render.ts` to `src/engine/segment-groups.ts` so both presentation layers share the same logic without crossing the web↔api boundary. Sub-minute `formatDuration` also fixed: 30s segments rendered as `"0min"`; now render as `"30s"`, `"1:30"` for mixed minute+second, `"4min"` for whole minutes.
- **Per-user `runRecommendationSource` flag** (`src/users.ts`, `UserProfile.runRecommendationSource?: "stryd"`). Set on ze, undefined on Pam. Drives the Stryd swap on every surface. Default behaviour unchanged for users without the flag.
- **`groupPairSegments` shared module** (`src/engine/segment-groups.ts`). Pure function over `WorkoutSegment[]` — extracts repeated (work, rest) × N patterns into pair groups. Conservative byte-equal matching (any field mismatch breaks the pair, so Hill Hustle's 3-segment work+rest+sprint blocks never collapse). Tests at `tests/engine/segment-groups.test.ts`.
- **`promus-dsw` env wiring** (`docker-compose.yml`, `.env.example`). `PROMUS_API` + `PROMUS_URL` env vars now thread through both `exercitator-mcp` and `praescriptor-web` containers. Defaults to `https://promus.tail7ab379.ts.net` when `PROMUS_URL` is unset.
- **`phase2/external-coach-integration-playbook.md`**. Process playbook capturing the Stryd-recs integration arc (Phases 0 → 7) so the same pattern can be repeated for FORM swim, MySwimPro, or any future external coach-AI bridge. Identifies the decision tree (does the vendor have an API? mitm capture vs published spec), the per-phase checklist (Phase 0 wire verification → Phase 1 client → Phase 2 mapper → Phase 3 swap on all surfaces → Phase 4 Promus logging → Phase 5 push + signals → Phase 6 UX adaptations → Phase 7 validation), and process meta-lessons (float-vs-int FTP drift; TZ bug in scheduleWorkout; SAST iteration spiral stopping criterion). Specific FORM-swim pre-Phase-0 step: mitm-capture the FORM iOS app to confirm whether it has a recommendations API beyond the existing NLP-script input.

### Changed
- **Stryd swap uses integer FTP from `power_context.ftp`** (`src/web/stryd-swap.ts`). `applyStrydRecommendation` previously took `ftp: number` as a parameter, passed in by callers as the raw Stryd CP float (`285.86…`). The engine rounds CP to integer at `src/engine/suggest.ts:171` and reports `power_context.ftp = 286` to every downstream consumer. The swap layer was using a different representation, causing replay-from-Promus to compute different bands than the live engine output. Fix: drop the `ftp` parameter, read `suggestion.power_context.ftp` internally. Visible behaviour change: bands like `286-314W` (from `round(110% × 285.86)`) become `286-315W` (from `round(110% × 286)`). Replay now byte-equal to live emission.

### Added (prior — Promus DSW emission, JSON cap, swap-hardening; before today's wider arc)
- **Promus DSW emission** (`src/web/promus-dsw.ts`). After every Stryd-attempted run prescription (success or fallback), Praescriptor fire-and-forgets a `POST /api/ingest/dsw` to Promus (`promus.tail7ab379.ts.net` by default, override via `PROMUS_URL`) carrying the full Stryd recommendation set, the engine's category decision, picked workout id/title/type/rationale, fallback reason if any, and an MVP `exercitator_context` block (`readiness_score`, `cp_or_ftp`, `power_source`, `vigil_severity`, `vigil_status`, `warnings_count`). The endpoint is idempotent on `(user_id, date, sport, source)` so re-renders upsert in place. Auth uses the existing `promus-api` bearer token from `.env`. Failures (network, 4xx, 5xx, missing token) log a `console.warn` and drop — the emitter NEVER blocks the prescription. Skips internally for engine-only prescriptions (Pam, rest days). 10 new vitest cases covering payload-building branches + emitter happy-path + missing-token + 4xx + network failure. Promus side landed as `zestuart/promus#165`. Deeper readiness-component plumbing (days_since_hard, fitness/fatigue/form, hrv_recent, sleep_debt_nights, cross_training_strain, staleness_tier) is TODO-flagged in `buildExercitatorContext` — JSONB schema accepts fields incrementally without a Promus migration.

### Security
- **Hardened the Stryd-swap layer against malformed/hostile recommendation payloads** (`src/web/stryd-swap.ts`). The `applyStrydRecommendation` function now performs pre-flight validation on the Stryd response before calling `strydWorkoutToSegments`, with explicit fallback paths (each surfaces a distinct `fallbackReason` on the card chip): `unsafe_block_count` (> 200 blocks), `unsafe_repeat_count` / `b.repeat` out of `[1, 100]` / non-integer, `unsafe_segments_per_block` (> 50), `unsafe_segment_duration` (any segment's hour > 24 h or minute/second ≥ 60), `malformed_duration_time` (null/non-object where `duration_type === "time"`), `unsafe_total_segment_count` (block × repeat × segs/block product > 500). The 1 MB JSON cap in `client.ts` (`MAX_RECOMMENDATIONS_JSON_BYTES`) is the load-bearing structural defence; these per-shape checks are belt-and-braces. Also bounded the `generatePrescriptions` in-memory user cache at `MAX_CACHE_ENTRIES = 100` with FIFO eviction (real registry has 2 entries today; the cap is for defensive completeness). Log-injection guard on the swap's `console.warn` strips `\r\n\t\v\f` from the error message before logging. All fixes derive from the 2026-05-25 SAST diff scan iterating against the new code. **Accepted findings**: Gemini's final pass flagged a theoretical stored-XSS via Stryd `title` — false positive, render.ts:299 already wraps every dynamic title in `escapeHtml`; the SAST diff bundle does not include render.ts so the existing sink was invisible to Gemini.
- **Generalised JSON-response size cap across `StrydClient`** (`src/stryd/client.ts`). New `parseBoundedJson<T>(res, label)` helper enforces `MAX_JSON_RESPONSE_BYTES = 1 MB` before `JSON.parse` on six methods that previously called `await res.json()` unbounded: `login`, `listActivities`, `downloadFit` metadata fetch (binary stream still uses `MAX_FIT_SIZE_BYTES`), `getLatestCriticalPower`, `createWorkout`, `scheduleWorkout`. Same defensive posture as the 2026-05-25 SAST-finding fix to `getRecommendedWorkouts`, generalised so a hostile or buggy Stryd upstream cannot exhaust memory through any single client method. 4 new vitest cases assert the cap fires across representative methods. `getRecommendedWorkouts` keeps its existing `MAX_RECOMMENDATIONS_JSON_BYTES` constant + bespoke error message (already covered by dedicated tests).
- **Defensive caps on Stryd-recommendations parsing** — both findings from the 2026-05-25 SAST diff scan resolved before deploy. `strydWorkoutToSegments` clamps `block.repeat` to `MAX_BLOCK_REPEAT = 100` (real workouts top out around 12 reps; the cap defends against a compromised/malformed upstream response that would otherwise flatten a billion-rep block into memory). `getRecommendedWorkouts` reads the response as text first and rejects payloads above `MAX_RECOMMENDATIONS_JSON_BYTES = 1 MB` before calling `JSON.parse` (observed payloads are 4–20 KB; 1 MB is generous headroom while capping the OOM blast radius from a hostile upstream). The Stryd JSON endpoints elsewhere in the client still use unguarded `res.json()` — same threat applies, fix deferred to a focused security pass.

### Added
- **Stryd workout recommendations as ze's run prescription source** (`src/web/stryd-swap.ts`, `src/engine/stryd-mapper.ts`, `src/stryd/client.ts`). When `runRecommendationSource: "stryd"` is set on the user profile (currently ze only), the engine's category decision still gates intensity — readiness, Vigil, HRV, sleep debt, cross-training, staleness all apply unchanged — but the *segments* are replaced by a Stryd-served workout chosen via `intensity_zones` overlap with the category target (NOT Stryd's `Best match` label, which rotates day-to-day per the 2026-05-25 Phase 0 verification). Recovery-day stride workouts are rejected so the athlete doesn't get 105% CP strides on a real recovery day. Failure modes — 204 on `type=long` (Pioneer subscription alone does not unlock the `long` bucket; needs an active adaptive plan), 5xx, 401, network errors, picker rejections — all fall back to the engine's own segments with the reason surfaced as a chip on the Praescriptor card (`Source: Exercitator (Stryd unavailable: <reason>)`). Successful swaps render a green chip (`Source: Stryd · <title>`). Pam's flow is untouched, swims are untouched, rest days short-circuit before the Stryd call. HTTP API (`src/api/types.ts`, `src/api/payload.ts`) exposes `prescription_source`, `fallback_reason`, `stryd_workout_id`, `stryd_workout_title`, `stryd_pick_rationale` so Excubitor / Nunc render the same attribution. 35 new vitest cases (7 client + 20 mapper + 8 swap).

### Changed
- Readiness score is now sport-aware and rebuild-aware (`src/engine/readiness.ts`). Three changes work together:
  - **TSB rebuild flag**: when `CTL < 30` AND `FTP/CTL > 12 W per CTL-point` (athlete returning from a layoff with intact power capacity), the TSB component is lifted to a floor of 60. Without it, low-CTL athletes' TSB component sat near 40 even on rested days because absolute TSB units (the existing ±20 lerp) ignored CTL — a TSB of −4 reads as "mildly fatigued" on CTL 60 and "almost recovered" on CTL 20, but the old engine treated them identically. The rebuild flag is surfaced as `ReadinessResult.rebuild: boolean`.
  - **Sport-specific recency**: `computeRecency` accepts an optional `sport` filter. Cross-sport activities (a swim, a ride) no longer suppress recency for a Run prescription — they showed up in the recency sigmoid even though they don't physiologically load the prescription's primary musculature. When sport-filtered and no qualifying activity exists in the window, recency returns 100 (fully rested for that sport). Without `sport`, behaviour matches the original (backward-compatible).
  - **Subjective renormalisation**: when ≥ 3 components have real data, missing components no longer default to NEUTRAL (50) and silently drag the score toward neutral. Weights renormalise across the present components. With < 3 components, fall back to the NEUTRAL-default behaviour (conservative when data is thin). Stops a missing `perceived_exertion` field from capping an otherwise-high readiness at ~85.
  - All three flow through `suggestWorkoutFromData` via a new optional `ReadinessOptions` param passed from `computeReadiness(wellness, activities, now, { ftp, sport })`. Existing callers without options see no behaviour change beyond the renormalisation rule for missing subjective data.

### Fixed
- Power-source detection now recognises Suunto + Stryd pod as Stryd-native (`src/engine/power-source.ts`). User-visible symptom: today's Praescriptor showed the warning "Power field is set to Garmin native but Stryd is connected. Zone targets converted using estimated 0.87 correction factor." despite the athlete running only on Stryd power. Root cause: most recent run was on a Suunto Vertical 2 paired with the Stryd pod; `isStrydNativeRecording` only recognised Apple Watch (via `/^Watch\\d/` device pattern + `*-Stryd.fit` filename) and the `"STRYD"` device-name upload case. Suunto activities have an opaque UUID `external_id` (no "stryd" substring) and only carry `StrydStepLength` in `stream_types` (not the CIQ triplet `hasStrydStreams` requires), so they fell through to the "Garmin active + athlete has Stryd" branch which applies a bogus 0.87 correction factor. Per-device heuristic split: AW keeps the filename-based detection (preserves the Stryd FIT enricher's HealthFit-vs-Stryd-app distinction); Suunto falls back to "any Stryd developer field in `stream_types`". Generalised the "no Stryd pod" warning to interpolate the actual `device_name` rather than hard-coding "Apple Watch native power".
- Run warm-ups capped at 6 minutes regardless of CTL (`src/engine/workout-builder.ts`). The threshold warm-up was the outlier — `scaled(900, scale)` produced 9:00 at low CTL, 22:30 at high CTL, both of which prompt watch-side "slow down" nags after the athlete is already warm. New `WARMUP_MAX_SECS = 360` constant and `scaledRunWarmup` helper wrap the seven run categories. Min-duration enforcement (`buildWorkout`) now uplifts non-warm-up segments only, so the cap survives floor-padding (previously a 1800 s tempo Run min would scale the warm-up back to 400 s through proportional uplift). Tests pin both the cross-category cap and the threshold-specific 540→360 shrink.
- `submit_cross_training_rpe` (MCP tool) and `POST /api/users/:userId/cross-training/:activityId/rpe` (HTTP API) now write `session_rpe` to intervals.icu in addition to `perceived_exertion`. The strain cascade's tier-2 check in `src/engine/cross-training-strain.ts` reads `session_rpe`, but the tools only ever wrote `perceived_exertion` — intervals.icu doesn't auto-derive one from the other, so submitting RPE did nothing for strain classification. New value is Foster's session-RPE: `Math.round(rpe × moving_time)`. Tool description also clarified that it accepts any recent activity, not just same-day. Closes a quiet bug where the user could submit RPE for yesterday's lift and find the next prescription still treating it as "unknown" strain.

### Added
- HTTP API v0.3 endpoints for the Excubitor (Nunc) iPhone client: `POST /api/users/:userId/push-to-stryd` (bearer-scoped wrapper around the existing `sendToStryd` Praescriptor function — same per-day dedup, same `?force=true` override, same response shape) and `GET /api/users/:userId/form-text` (returns today's swim prescription as FORM-goggles Script `text/plain`, lifted from `buildFormDescription`). Both routes share state with Praescriptor's buttons, so a push from one surface is visible to the other. Spec: `phase3/exercitator-http-api-v0.3-delta.md`.

### Security
- HTTP API `tz` validation extracted to `src/api/tz.ts:resolveTz` and applied uniformly across all four `tz`-consuming handlers (`status`, `workouts/*`, `dashboard`, `push-to-stryd`, `form-text`). Closes a high-severity DoS regression on `/dashboard` where the previous `q?.includes("/")` check let a crafted `?tz=a/a` reach `localDateStr` → `Intl.DateTimeFormat` → `RangeError` → process crash. The other handlers were already correct; centralising the resolver prevents the same finding recurring on the next handler.
- MCP `submit_cross_training_rpe` tool now applies the same `^[A-Za-z0-9_-]{1,64}$` allowlist to `activityId` that the HTTP API equivalent uses (`isValidIntervalsId`). Closes a high-severity SSRF/path-traversal vector where a crafted Claude-supplied id could reach the upstream URL constructor before `encodeURIComponent` defended.

### Fixed
- `criticalPowerFromContext` (HTTP API `/status` and `/dashboard` CP block) now picks Stryd CP over `powerContext.ftp` when both are present (closes #32, regression of #31). The previous precedence (`powerContext.ftp > 0 ? powerContext.ftp : strydCp`) shipped intervals.icu rolling FTP under a `stryd_direct` label because these two handlers build their `powerContext` directly from `detectPowerSource()` and never route through `suggestWorkoutFromData`'s engine-level Stryd override. The #31 close note ("`watts === Math.round(strydCp.cp)` by construction") was wrong — it accounted only for the suggestion engine path, not the parallel status/dashboard path. New precedence (`strydCp ?? (powerContext.ftp > 0 ? powerContext.ftp : null)`) makes the function itself enforce the same authority that the suggestion engine does, regardless of which handler called it. Stryd-less users (Garmin / intervals.icu only) keep the rolling-FTP fallback unchanged. Regression test pins the `(ftp=315, strydCp=286) → watts=286` shape that was producing the bug live.
- intervals.icu run workout description now emits absolute watts end-to-end when a power source is configured. `formatRunTarget` previously rendered explicit power bands as `%FTP` (e.g. `80-90%`) and warm-up / cool-down segments without a power band fell through to HR (e.g. `72-82% HR`); the inter-rep recovery line was hard-coded to `50%`. All three paths now produce wattage ranges, with warm-up / cool-down / recovery synthesising a Stryd Z1 Easy 65–80% CP band from FTP when the segment carries no explicit watts. HR fallback retained for athletes without a power source. Vitest pin in `tests/web/intervals-format.test.ts` includes an end-to-end ASCII-only assertion to catch any future stray non-hyphen-minus characters at the formatter level (the actual em-dash users see on intervals.icu's web UI is an editor-side auto-correct, not something we send).
- `POST /api/users/:userId/push-to-stryd` 409 response now emits `workout_id` as a number, matching the 200 path. Previously the 409 path returned the SQLite-stored string representation (the storage layer coerces to TEXT for the generic `external_id` column at `src/web/send-stryd.ts:77`, and the response read-back at `:38` surfaced it unchanged), breaking type-strict clients (Excubitor M11.6 hit this in production on first push). Spec §2.1.3 §2 corrected and a 200/409 contract test added.
...
- (See Fixed entry above for the #32 follow-up that fully closes the `watts`/`source` divergence story. The #31 Changed entry below describes the engine-side fix that was the first half of the resolution; #32 was the second half.)

### Changed
- Engine FTP resolution now trusts Stryd CP unconditionally when present (closes #31). The 2026-05-01 staleness override (rolling-FTP fallback when Stryd CP > 30d old AND inferred FTP ≥ 5% higher, plus the softer "consider a CP test" warning) is removed. Reasons: (a) it created the only path on which `critical_power.watts` could disagree with `critical_power.source`, since `mapWirePowerSource` reported `stryd_direct` whenever the Stryd query succeeded regardless of which value won — Excubitor's iOS card showed an FTP that disagreed with Praescriptor's header even though both were nominally "stryd_direct"; (b) post-layoff CP staleness is a calibration problem, not a numbers-game the engine should second-guess. Athlete is on the hook to book a fresh CP test when fitness shifts. `STRYD_CP_STALE_DAYS` and `STRYD_CP_STALE_OVERRIDE_RATIO` constants deleted; warning strings deleted; `StrydCpInput.ageDays` retained because the HTTP API `critical_power.updated_at` and Praescriptor's data-source row still surface CP age to the human in the loop.
- HTTP API v0.3 spec §2.1.3 (`phase3/exercitator-http-api-v0.3-delta.md`) corrected — `workout_id` and `calendar_id` documented as numeric (integer) on both 200 and 409 paths. Original example showed both as strings; truth-of-source against `StrydClient.createWorkout` (`src/stryd/client.ts:236`) and `StrydCalendarEntry.id` (`src/stryd/client.ts:69`) is `number` for both. Added a typed field table covering `success`, `workout_id`, `calendar_id`, `stress`, `duration_mins`, `distance_m`, `duplicate`, `message`, `error`. Resolution of the 2026-05-03 Excubitor amendment proposal — see `phase3/exercitator-http-api-v0.3-amendment-proposal-2026-05-03.md` §7.
