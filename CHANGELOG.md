...
- HTTP API response cache now bounded — LRU eviction at `EXERCITATOR_API_CACHE_MAX_ENTRIES` (default 1000) plus a 60 s background prune of already-expired entries. Prevents an authenticated tailnet client from filling memory with crafted cache keys (`src/api/cache.ts`, `src/api/server.ts`).
...
- HTTP API `/workouts/suggested` cache key now allowlists the `sport` query parameter to {`auto`, `Run`, `Swim`} — an arbitrary tainted value can no longer reach the in-memory cache (`src/api/handlers/workouts.ts`).
