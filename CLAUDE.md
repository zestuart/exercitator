...
- **SAST baseline**: Last clean tag is `sast-baseline-2026-05-09` on commit `0ff5171` (2026-05-09 deploy removing the Stryd CP staleness override — issue #31). Earlier baselines: `sast-baseline-2026-05-08` (commit `7d575b2`, run-watts format fix), `sast-baseline-2026-04-29-c` (commit `25db117`, before the two High findings were closed in the 2026-05-03 deploy). `python3 scripts/sast_scan.py --mode diff` scans only files changed since the latest baseline. Re-baseline immediately after each clean deploy or accepted-risk deploy.
