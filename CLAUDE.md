...
**Current baseline**: `sast-baseline-2026-05-27-b` on commit `9e2d2fc` (same-sport already-trained Quies suppression card + API 0.2.0: engine short-circuits before Vigil/category when an activity matching the requested sport already exists today; Stryd/FORM swap layers gate on the new status so no upstream vendor call fires; HTTP API surfaces `status: "already_trained"` + `rest_message` block with server-rendered Quies invocation). Accepted findings: pre-existing `void emitDsw(...)` "DoS via unhandled rejection" pattern (emitDsw is structurally bulletproof via internal `.catch`) and false-positive XSS via `userId` in `clientJs` (profile.id is a closed enum from `src/users.ts:PROFILES`, not user-controlled — unknown slugs return 404 before `clientJs` runs). Earlier today: `sast-baseline-2026-05-27` on `c9dc2bd` (Phase 7 replay closed-loop via Promus #167: new `fetchDswRecord` read helper + scaffold HTTP mode + `swim_css_m_per_s` persisted into `exercitator_context` + shared `validateFormWorkoutBody` defends both the swap layer and replay against poisoned DSW rows). Earlier today: `sast-baseline-2026-05-26-b` on `fad8d6b`, `sast-baseline-2026-05-26` on `ec2b6ff`. Earlier baselines: `sast-baseline-2026-05-25-d` on `9d3ce13`, `sast-baseline-2026-05-25-c` on `5fc31b0`, `sast-baseline-2026-05-25-b` on `53f063e`, `sast-baseline-2026-05-25` on `9960fc3`. Accepted finding from prior -d diff: pre-existing hardcoded `"0"` for Swim userId at `prescriptions.ts:118` — `"0"` is a Vigil-disable sentinel (Vigil is Run-only), not a real user id; cross-user leak is structurally impossible because upstream `data` is fetched per-user. **For future external-coach integrations** see `phase2/external-coach-integration-playbook.md` (Stryd run + FORM swim are the canonical reference arcs). `python3 scripts/sast_scan.py --mode diff` scans only files changed since this tag. Re-baseline immediately after each clean deploy or accepted-risk deploy.
