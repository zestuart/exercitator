...
**Current baseline**: `sast-baseline-2026-05-27` on commit `c9dc2bd` (Phase 7 replay closed-loop via Promus #167: new `fetchDswRecord` read helper + scaffold HTTP mode + `swim_css_m_per_s` persisted into `exercitator_context` + shared `validateFormWorkoutBody` defends both the swap layer and replay against poisoned DSW rows). Verified end-to-end: live FORM/Swim DSW row replayed via `scripts/replay-form-dsw.ts --user ze --date YYYY-MM-DD --sport Swim --source form` produces SHA-256 byte-equal to the determinism-guard inline snapshot. Earlier today: `sast-baseline-2026-05-26-b` on `fad8d6b`, `sast-baseline-2026-05-26` on `ec2b6ff`. Earlier baselines: `sast-baseline-2026-05-25-d` on `9d3ce13`, `sast-baseline-2026-05-25-c` on `5fc31b0`, `sast-baseline-2026-05-25-b` on `53f063e`, `sast-baseline-2026-05-25` on `9960fc3`. Accepted finding from prior -d diff: pre-existing hardcoded `"0"` for Swim userId at `prescriptions.ts:118` — `"0"` is a Vigil-disable sentinel (Vigil is Run-only), not a real user id; cross-user leak is structurally impossible because upstream `data` is fetched per-user. **For future external-coach integrations** see `phase2/external-coach-integration-playbook.md` (Stryd run + FORM swim are the canonical reference arcs). `python3 scripts/sast_scan.py --mode diff` scans only files changed since this tag. Re-baseline immediately after each clean deploy or accepted-risk deploy.
