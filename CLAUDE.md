...
**Current baseline**: `sast-baseline-2026-05-26-b` on commit `fad8d6b` (FORM `picked_workout_body` now persisted into `exercitator_context` for byte-equal replay-from-Promus once #167 ships; plus `emitDsw` log-injection sanitisation + bounded read of Promus error bodies via new `readBoundedText`. Two infrastructure findings from iteration-3 accepted with rationale per the SAST-spiral policy: Tailscale key reuse across sidecars is deliberate per praefectura/docs/tailscale.md; PROMUS_API on `exercitator` is required because the HTTP API handlers emit DSW from there). Earlier today: `sast-baseline-2026-05-26` on `ec2b6ff`. Earlier baselines: `sast-baseline-2026-05-25-d` on `9d3ce13`, `sast-baseline-2026-05-25-c` on `5fc31b0`, `sast-baseline-2026-05-25-b` on `53f063e`, `sast-baseline-2026-05-25` on `9960fc3`. Accepted finding from prior -d diff: pre-existing hardcoded `"0"` for Swim userId at `prescriptions.ts:118` — `"0"` is a Vigil-disable sentinel (Vigil is Run-only), not a real user id; cross-user leak is structurally impossible because upstream `data` is fetched per-user. **For future external-coach integrations** see `phase2/external-coach-integration-playbook.md` (Stryd run + FORM swim are the canonical reference arcs). `python3 scripts/sast_scan.py --mode diff` scans only files changed since this tag. Re-baseline immediately after each clean deploy or accepted-risk deploy.
