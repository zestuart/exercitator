...
**Current baseline**: `sast-baseline-2026-05-25-c` on commit `5fc31b0` (full Stryd recommendation arc: round-trip push to Stryd calendar + scheduleWorkout TZ fix + PATCH selected_id signal on every send + Stryd swap on all Run-producing API surfaces (/workouts/suggested + /dashboard) + engine narrative stripped on Stryd-sourced (rationale = Stryd desc, terrain neutralised, staleness/buffer warnings filtered) + work+rest pair-collapse mirrored on the API surface + ApiSegment.rest_target/rest_target_description added + groupPairSegments lifted to src/engine/segment-groups.ts so api and web share the same logic without crossing layers). Earlier baselines: `sast-baseline-2026-05-25-b` on `53f063e`, `sast-baseline-2026-05-25` on `9960fc3`. (Stryd workout-recommendations integration phases 1–3 + defensive caps on `block.repeat` and recommendation-response size, both from the 2026-05-25 SAST findings + sast_scan.py inline-content patch for cache-disabled accounts). `python3 scripts/sast_scan.py --mode diff` scans only files changed since this tag. Re-baseline immediately after each clean deploy or accepted-risk deploy.
